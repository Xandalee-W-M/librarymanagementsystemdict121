...
if (!$user_stmt) {
    die("User details query preparation failed: " . $conn->error);
}
...
$borrowed_books_stmt = $conn->prepare(
    "SELECT bb.id, b.title, b.author, bb.borrowed_at, bb.due_date, bb.returned_at
     FROM borrowed_books bb 
     JOIN books b ON bb.book_id = b.id 
     WHERE bb.user_id = ?"
);
if (!$borrowed_books_stmt) {
    die("Borrowed books query preparation failed: " . $conn->error);
}
...
$books_stmt = $conn->query("SELECT id, title, author, isbn, available FROM books");
if (!$books_stmt) {
